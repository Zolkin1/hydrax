...
class HumanoidHighLevel(Task):
    """Standup task for the Unitree G1 humanoid."""

    def __init__(self, u_max: jax.Array, u_min: jax.Array, target_pos: jax.Array) -> None:
        """Load the MuJoCo model and set task parameters."""
        mj_model = mujoco.MjModel.from_xml_path(
            ROOT + "/models/g1/g1_21j_scene.xml"
        )
        super().__init__(
            mj_model,
            trace_sites=["imu_in_torso"],
        )
    ...
    def _get_torso_xy_position(self, state: mjx.Data) -> jax.Array:
        """Get the torso position."""
        return state.qpos[:2]

    def running_cost(self, state: mjx.Data, control: jax.Array) -> jax.Array:
        """The running cost ℓ(xₜ, uₜ)."""
        position_cost = jnp.square(self._get_torso_xy_position(state) - self.target_pos)
        input_cost = jnp.sum(jnp.square(control))
        return 1 * position_cost + 0.01 * input_cost
